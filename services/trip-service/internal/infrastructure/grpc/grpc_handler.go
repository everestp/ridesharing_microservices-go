...
func (h *gRPCHandler) PreviewTrip( ctx context.Context,  req *pb.PreviewTripRequest) (*pb.PreviewTripResponse, error) {
   ...
   userID := req.UserID
	rout, err := h.service.GetRoute(ctx,pickupCoord , destinationCoord)
	if err != nil {
		log.Println(err)
		return nil, status.Errorf(codes.Internal ,"Failed to get route : %v",err)
		}
// 1 .Estimate the ride  fare prices  based on the route ( eg distance)
estimatedFare := h.service.EstimatePackagesPriceWithRoute(rout)
//2.Store the ride date for the create trip 
fare ,err := h.service.GenerateTripFares(ctx, estimatedFare, userID ,rout)
if err != nil {
